...
DcfManager::IsBusy (void) const
{
    ...
    { 
	 /* if(Simulator::Now ()>Seconds(tempsBascule))
	  {   
		  ...
		  double randDouble = gen->GetValue(0,0.1);
		  int randInt = int(randDouble*3000000000);
		  m_phyListener->NotifySleep();
		  MY_DEBUG(" sleep time rxing: "<<NanoSeconds(randInt));
		  Simulator::Schedule(NanoSeconds(randInt),&PhyListener::NotifyWakeup, m_phyListener);
	  }*/
      return true;
    }
  ...
  if (lastCCABusyEnd > Simulator::Now ())
    {
	 /* if((non_persistent==1)&&(Simulator::Now ()>Seconds(tempsBascule)))
	  {   
		  Ptr<UniformRandomVariable> gen = CreateObject<UniformRandomVariable>(); 
		  double randDouble = gen->GetValue(0,0.1);//(0,0.1) best results so far
		  int randInt = int(randDouble*500000000);//300000000 best results sot far 
		  m_phyListener->NotifySleep();
		  MY_DEBUG(" sleep time ccabusy: "<<NanoSeconds(randInt));
		  if(m_retryLimit<limit_tentative)
		  {
		    m_retryLimit = m_retryLimit + 1;
		  }
		  else
		  {
		    NS_LOG_DEBUG ("packet drop");
		  }
		  Simulator::Schedule(NanoSeconds(randInt),&PhyListener::NotifyWakeup, m_phyListener);
	  }*/
      return true;
...
DcfManager::RequestAccess (DcfState *state)
{
  ...
  if (state->GetBackoffSlots () == 0)
    {
	   MY_DEBUG (" backoffslot:"<<state->GetBackoffSlots ());
      if (IsBusy ())
        {
          MY_DEBUG ("medium is busy: collision"<<std::endl);
