...
MacLow::GetAckTimeout (void) const
{
  if(noAck==1)	
  {
	...
	double randAckDouble = gen->GetValue(0,0.3);
	int randAck = int(randAckDouble*100);
	std::cout<<Simulator::Now()<<" acktimeout: "<<MilliSeconds(randAckDouble*100)<<" random: "<<randAck<<std::endl;
	if(Simulator::Now()>Seconds(tempsBascule))  
	{  return (MilliSeconds(randAckDouble*100)); }
	else {return m_ackTimeout;}
  }	  
  else
  { 
	return m_ackTimeout; 
                           ...
                           MacLowTransmissionListener *listener)
{
  ...
  CancelAllEvents ();
  m_listener = listener;
  Mac48Address sourceAddr = hdr->GetAddr1();
  if((noAck==1)&&(Simulator::Now()>Seconds(tempsBascule))&&((maxRetransmission==0)||(m_stationManager->GetRetryCount(sourceAddr,hdr)+1==maxRetransmission)))
  {
    params.DisableAck();
  }//abk: pour que le noeud n'attend pas un ack et lance le missedack() 
  /*Ptr< WifiPhy > myPhy = this->GetPhy();
  Ptr<NetDevice> thisDevice= DynamicCast<NetDevice>(myPhy->GetDevice());
  int nodeID = -500;
  ...
  if(Simulator::Now()>Seconds(tempsBascule))
  {	
	NS_LOG_DEBUG (Simulator::Now()<<" Node["<<std::to_string(nodeID)<<"] macLow StartTransmission"); 
  }*/
  m_txParams = params;
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, WifiPreamble preamble, bool ampduSubframe)
{
  ...
  if (hdr.IsRts ())
    ...
    {
      NS_LOG_DEBUG ("receive ack from=" << m_currentHdr.GetAddr1 ()<<" rxSnr: "<<rxSnr<<" mode:"<<txVector.GetMode());
      SnrTag tag;
      packet->RemovePacketTag (tag);
      m_stationManager->ReportRxOk (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                    rxSnr, txVector.GetMode ());
          ...
          if (hdr.IsQosAck () && !ampduSubframe)
            {
              ...
              NS_ASSERT (m_sendAckEvent.IsExpired ());
              //ab
              if((noAck==1)&&(Simulator::Now () > Seconds(tempsBascule)))
              {
                NS_LOG_DEBUG(Simulator::Now ()<<" mac low no ack creation!!!");
              }
              else
              {//k
                m_sendAckEvent = Simulator::Schedule (GetSifs (),
      ...
      if (firsthdr.GetAddr1 () == m_self)
        {
          ...
          if (firsthdr.IsAck () || firsthdr.IsBlockAck () || firsthdr.IsBlockAckReq ())
          ...
          else if (firsthdr.IsData () || firsthdr.IsQosData ())
            {
               if(Simulator::Now () > Seconds(tempsBascule))
              {
                NS_LOG_DEBUG(Simulator::Now ()<<" ReceiveOk ack block!!!");
              }
              else
              {
