...
WifiRemoteStationManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiRemoteStationManager")
    ...
    .AddAttribute ("MaxSlrc",
                   ...
                   "This value will not have any effect on some rate control algorithms.",
                   UintegerValue (maxRetransmission),
                                        ...
                                        double ackSnr, WifiMode ackMode, double dataSnr)
{
  ...
  Ptr<NetDevice> thisDevice= DynamicCast<NetDevice>(m_wifiPhy->GetDevice());
  ...
  {
    sleeping = " GoBackToSleep";
    arpEnd = " arpEnd"; 
  }
  NS_LOG_DEBUG (Simulator::Now ()<<" WifiRemoteStationManager:: Node["<<nodeID<<"] retry counter: "<<station->m_slrc<<arpEnd<<sleeping);
  station->m_slrc = 0;
                                   ...
                                   Ptr<const Packet> packet, WifiTxVector txVector)
{
  ...
  if (m_protectionMode == RTS_CTS
    ...
    {
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::NeedRTS returning true to protect non-ERP stations");
      return true;
    }
  if (address.IsGroup ())
    {
...
WifiRemoteStationManager::NeedCtsToSelf (WifiTxVector txVector)
{
  ...
  if (m_protectionMode == CTS_TO_SELF
      ...
      && m_useNonErpProtection)
    {
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::NeedCtsToSelf returning true to protect non-ERP stations");
      return true;
    }
    ...
    //search for the BSS Basic Rate set, if the used mode is in the basic set then there is no need for Cts To Self
      for (WifiModeListIterator i = m_bssBasicRateSet.begin (); i != m_bssBasicRateSet.end (); i++)
        {
          if (mode == *i)
            {
              NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::NeedCtsToSelf returning false");
              return false;
            }
        }
      if (HasHtSupported ())
      ...
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::NeedCtsToSelf returning true");
      return true;
    }
  return false;
}
                                                 ...
                                                 Ptr<const Packet> packet)
{
  ...
  NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::NeedDataRetransmission count: " << station->m_ssrc << " result: " << std::boolalpha << normally);
                                                  ...
                                                  Ptr<const Packet> packet)
{
  ...
  if((station->m_slrc == maxRetransmission) && (tempsActuel > Seconds(tempsBascule)))
  {
    sleeping = " GoBackToSleepFrameDrop";
  }
  if(Simulator::Now ()>tempsBascule){ NS_LOG_DEBUG (Simulator::Now ()<<" WifiRemoteStationManager:: Node["<<nodeID<<"] ["<<addrMacSource<<"] to ["<< address <<"] NeedDataRetransmission count: " << station->m_slrc << " result: " << std::boolalpha << normally<<sleeping);}
                                             ...
                                             Ptr<const Packet> packet)
{
  ...
  NS_LOG_DEBUG (Simulator::Now ()<<" WifiRemoteStationManager::NeedFragmentation result: " << std::boolalpha << normally);
...
WifiRemoteStationManager::GetNFragments (const WifiMacHeader *header, Ptr<const Packet> packet)
{
  ...
  NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetNFragments returning " << nFragments);
  return nFragments;
}
                                           ...
                                           Ptr<const Packet> packet, uint32_t fragmentNumber)
{
  ...
  if (fragmentNumber >= nFragment)
    {
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetFragmentSize returning 0");
      return 0;
    }
  //Last fragment
  if (fragmentNumber == nFragment - 1)
    {
      ...
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetFragmentSize returning " << lastFragmentSize);
  ...
  //All fragments but the last, the number of bytes is (Threshold - WIFI_HEADER_SIZE - WIFI_FCS).
  else
    {
      ...
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetFragmentSize returning " << fragmentSize);
      return fragmentSize;
    }
}
                                             ...
                                             Ptr<const Packet> packet, uint32_t fragmentNumber)
{
  ...
  NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetFragmentOffset returning " << fragmentOffset);
                                          ...
                                          Ptr<const Packet> packet, uint32_t fragmentNumber)
{
  ...
  NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::IsLastFragment returning " << std::boolalpha << isLast);
  return isLast;
}
...
WifiRemoteStationManager::GetControlAnswerMode (Mac48Address address, WifiMode reqMode)
{
  ...
  if (found)
    {
      NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::GetControlAnswerMode returning " << mode);
      return mode;
    }
...
WifiRemoteStationManager::LookupState (Mac48Address address) const
{
  ...
  for (StationStates::const_iterator i = m_states.begin (); i != m_states.end (); i++)
    {
      if ((*i)->m_address == address)
        {
          //NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::LookupState returning existing state");
          return (*i);
        }
    }
  ...
  //NS_LOG_DEBUG (Simulator::Now ()<<"WifiRemoteStationManager::LookupState returning new state");
  return state;
}
//faut comprendre les deux fonctions lookup juste après ;)
WifiRemoteStation *
...
{
  //NS_LOG_DEBUG (Simulator::Now ()<<"DoReportAmpduTxStatus received but the manager does not handle A-MPDUs!");
}
...
//ab
uint32_t
WifiRemoteStationManager::GetRetryCount (Mac48Address address, const WifiMacHeader *header) 
{
  WifiRemoteStation *station = Lookup (address, header);
  return station->m_slrc;
}
