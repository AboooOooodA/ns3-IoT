...
DcaTxop::Queue (Ptr<const Packet> packet, const WifiMacHeader &hdr)
{
  ...
  //ab	
	 ...
	 int value;
     Mac48Address senderMacDest = hdr.GetAddr2();
     Mac48Address destMacDest = hdr.GetAddr1();	
     if(destMacDest.IsBroadcast() == false)
     {
       std::ostringstream stream;
       stream  << senderMacDest;
       std::string nodeSender = stream.str();
       nodeSender.erase(std::remove(nodeSender.begin(), nodeSender.end(), ':'), nodeSender.end());
  ...
  if((tempsActuel > Seconds(tempsBascule))&&(pcktDATA.compare(hdr.GetTypeString())==0))	
  {	//k
    NS_LOG_DEBUG(Simulator::Now ()<<" Node["<<senderNodeID<<"] DcaTxop Enqueed"<<" size: "<<m_queue->GetSize());
  }
  StartAccessIfNeeded ();
}
  ...
  if (m_currentPacket == 0)
    {
      ...
      m_fragmentNumber = 0;
      NS_LOG_DEBUG (Simulator::Now()<<" dequeued size=" << m_currentPacket->GetSize () <<
...
DcaTxop::MissedAck (void)
{
  ...
  NS_LOG_FUNCTION (this);
  NS_LOG_DEBUG (Simulator::Now()<<"missed ack"<<" cw "<<m_dcf->GetCw ());

    if (!NeedDataRetransmission ())
      {
        NS_LOG_DEBUG (Simulator::Now()<<"Ack Fail");
