...
WifiNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  /*int nodeID = -500;
  Ptr<Node> thisNode = DynamicCast<Node>(this->GetNode());
  if(thisNode != NULL) 
  {
   nodeID= thisNode->GetId ();
  }
  NS_LOG_DEBUG(Simulator::Now ()<<" Node["<<nodeID<<"] packet in mac queue (Send)");*/
  return true;
}
...
WifiNetDevice::SendFrom (Ptr<Packet> packet, const Address& source, const Address& dest, uint16_t protocolNumber)
{
  ...
  //NS_LOG_DEBUG(Simulator::Now ()<<" packet in mac queue (SendFrom)");
  return true;
}
