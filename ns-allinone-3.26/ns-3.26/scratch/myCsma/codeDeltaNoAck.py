...
fichier = open("results/Threshold"+str(Threshold)+"/"+model+"N"+str(nbNode)+distance+nbRetransmission+pcktSize+traffic+seed+".out", "r")
...
for line in fichier:
	...
	if ("DcaTxop Enqueed" in line):
		resStream.write(line.split(" ")[1]+" "+line.split(" ")[2]+" "+line.split(" ")[4]+"\n")
	if ("Transmitting DATA frame PHY layer to Node[0]" in line and "arpEnd" in line):
		resStream.write(line.split(" ")[0]+" "+line.split(" ")[1]+" "+line.split(" ")[3]+"\n")
		#print line#.split(" ")[0]+" "+line.split(" ")[1]
	if ("WifiRemoteStationManager::" in line and "NeedDataRetransmission" in line):
		resStream.write(line.split(" ")[0]+" "+line.split(" ")[2]+" "+line.split(" ")[6]+"\n")
	if ("ON APP" in line):
	...
	'''if ("Node[0] [00:00:00:00:00:01] Receive OK FOR ME from" in line and "DATA" in line and "arpEnd" in line):
		resStream.write(line.split(" ")[0]+" "+line.split(" ")[8]+" "+line.split(" ")[-1]+"\n")'''
	if ("Node[0] APPLICATION LEVEL Received" in line and "OFF APP" in line):
		resStream.write(line.split(" ")[0]+" "+line.split(" ")[8]+" "+line.split(" ")[14]+" "+line.split(" ")[-1]+"\n")
...
resStream2.write("\nCsma No Ack Nodes: "+str(nbNode)+" "+distance+" "+pcktSize+" "+nbRetransmission+" "+traffic+" "+seed+"\n")
print("\n *** Csma No Ack Nodes: "+str(nbNode)+" "+distance+" "+pcktSize+" "+nbRetransmission+" "+traffic+" "+seed+" ***") 
...
'''for i in range(nbNode):
	...
	if cpt==cpt2:
		...
		if cpt!=0:
			...
			print "awake time Node["+str(i)+"]: "+ str(deltaMeanTable[i])+"ns"
'''	
testStream=open("test.out","w")
testStream.write(str(nbNode)+" "+seed+"\n")
for i in range(nbNode):
	readStream=open("results/Trash/"+resultFile,"r")
	listOn=[]
	listSleep=[]
	typeSleep=[]	
	for line in readStream:
		if "["+str(i)+"]" in line and "Enqueed" in line:
			listOn.append(float(line.split(" ")[0].replace("+","").replace("n","").replace("s","")))
		elif "["+str(i)+"]" in line and "NeedDataRetransmission" in line:
			listOn.append(float(line.split(" ")[0].replace("+","").replace("n","").replace("s","")))
		elif "["+str(i)+"]" in line and "Transmitting" in line:
			listSleep.append(float(line.split(" ")[0].replace("+","").replace("n","").replace("s","")))
			typeSleep.append(1)

	testStream.write("\n Node["+str(i)+"]o f t: "+str(len(listOn))+"  "+str(len(listSleep))+"  "+str(len(typeSleep))+"\n")
	cpt=0
	somme=0
	#print "listOn len:"+str(len(listOn))+"\n"
	#print "listSleep len:"+str(len(listSleep))+"\n"

	for j in range(len(typeSleep)):
		if typeSleep[j]==1:
			cpt=cpt+1
			somme=somme+listSleep[j]-listOn[j]
			#somme=somme+(228000*int(sys.argv[4])) #txduration
...
for k in range(nbNode):
	if k==0: 
		...
		resStream2.write("\nawake time avg Node["+str(k)+"]: "+str(deltaMeanTable[k]/float(1000000))+" ms"+"\n")
...
for i in range(nbNode):
	...
	for line in readStreamEnd:
		if "["+str(i)+"]" in line and "WakingUp" in line:
			listOn.append(float(line.split(" ")[0].replace("+","").replace("n","").replace("s","")))
		elif "["+str(i)+"]" in line and "OFF APP" in line:
			listSleep.append(float(line.split(" ")[0].replace("+","").replace("n","").replace("s","")))
			typeSleep.append(1)
	...
	for j in range(len(typeSleep)):
		if typeSleep[j]==1:
			...
			if((listSleep[j]-listOn[j])<(float(sys.argv[3])*1000000000)):
				cpt=cpt+1				
				somme=somme+listSleep[j]-listOn[j]
			#somme=somme+listSleep[j]-listOn[j]
...
for k in range(nbNode):
	if k==0: 
		...
		testStream.write("\nEndToEnd Delay avg Node["+str(k)+"]: "+str(endToEndDelay[k]/float(1000000))+"ms"+"\n")'''
resStream2.close()
testStream.close()
