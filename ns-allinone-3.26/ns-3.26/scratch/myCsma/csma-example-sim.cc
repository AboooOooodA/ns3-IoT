...
bool inTable(int tab[], int length, int elem)
{
...
int main (int argc, char *argv[]) {
 ...
 for(int i=0;i<MAX;i++){ std::cout<<tab[i]<<" ";}
 std::cout<<std::endl; 
  ...
  cmd.AddValue("nbrNodes", "Total number of nodes in the network.",
              nbrNodes);
  cmd.AddValue("seedNumber", "The value of the seed.",
              seedNumber);
  //k
  ...
  //LogComponentEnable("WifiPhy",LOG_LEVEL_INFO);  => pas de résultats de ce log
  //LogComponentEnable("Application",LOG_LEVEL_INFO); => pas de résultats de ce log
  //LogComponentEnable("InterferenceHelper",LOG_LEVEL_INFO); => pas de résultats de ce log
  ...
  LogComponentEnable("DcfManager",LOG_LEVEL_INFO); //=> pour le CCA !!!!!!!!!!!!
  ...
  LogComponentEnable("ArpCache",LOG_LEVEL_INFO);
  ...
  //LogComponentEnable("AmrrWifiRemoteStation",LOG_LEVEL_INFO); => pas de résultats de ce log
  //LogComponentEnable("ArfWifiManager",LOG_LEVEL_INFO); => pas de résultats de ce log
  ...
  WifiHelper wifi;
  wifi.SetRemoteStationManager ("ns3::ConstantRateWifiManager");//,"DataMode", StringValue ("OfdmRate12Mbps"), "ControlMode", StringValue ("OfdmRate12Mbps"));
...
//TxPower
  ...
  wifiPhy.Set("TxPowerEnd",DoubleValue(39));
//TxPower

  YansWifiChannelHelper wifiChannel = YansWifiChannelHelper::Default ();
  //wifiChannel.AddPropagationLoss("ns3::FriisPropagationLossModel","Frequency",DoubleValue(5.15e9),"SystemLoss",DoubleValue (1),"MinLoss",DoubleValue (0));
  //YansWifiChannelHelper::SetPropagationDelay("ns3::RandomPropagationDelayModel");
  ...
  {
          //Wifi Remote Manager
          Ptr<WifiNetDevice> ApD= DynamicCast<WifiNetDevice>(nodeDevices.Get(i));
          Ptr<WifiRemoteStationManager> ApManager=ApD->GetRemoteStationManager();


          //NUMBER OF RETRANSMISSION
          ApManager->SetMaxSlrc(maxRetransmission);
          //if((dynCca==1)&&(inTable(tab,MAX,i))){ApManager->SetMaxSlrc(3);}
          std::cout<<std::endl<<"Node["<<i<<"] Number of retransmission: "<<ApManager->GetMaxSlrc()<<std::endl;

          //RTS/CTS THRESHOLD
          UintegerValue thresh= UintegerValue(5000);

          //ApManager->SetRtsCtsThreshold(thresh);
          Config::SetDefault("ns3::WifiRemoteStationManager::RtsCtsThreshold",thresh);
          std::cout<<"RTS/CTS Threshold: "<<ApManager->GetRtsCtsThreshold()<<std::endl<<std::endl;

  }

  //CCA THRESHOLD
  for(int i=1;i<nbrNodes;i++)//à partir de 0 juste quand noeud zero est emetteur sinon de 1
  {
    Ptr<WifiNetDevice> ApDC= DynamicCast<WifiNetDevice>(nodeDevices.Get(i));
    Ptr<WifiPhy> wfManager=ApDC->GetPhy();
    //wfManager->SetEdThreshold(-17);
    //wfManager->SetCcaMode1Threshold(-90);      //ligne à mettre en commentaire pour activer le CCA (aloha -> csma) + la boucle for suivante
    if((dynCca==1)&&(inTable(tab,MAX,i)))
    {
     //wfManager->SetEdThreshold(-96);
     wfManager->SetCcaMode1Threshold(-90);
    }  
    std::cout<<"ccaMode1Threshold Node["<<i<<"]: ccaMode1Threshold("<<wfManager->GetCcaMode1Threshold()<<") EdThreshold("<<wfManager->GetEdThreshold()<<")"<<std::endl<<std::endl;
  }

  //Ptr<UniformRandomVariable> ackGen = CreateObject<UniformRandomVariable>(); 

  //BACKOFF CONTENTION WINDOW  
 /* for(int i=0;i<nbrNodes;i++)
  {
    //if((dynCca==1)&&(inTable(tab,MAX,i)))//===> oublier pas de la mettre en commentaire si pas de CCA Dynamique
    { 
      Ptr<WifiNetDevice> wifi_dev = DynamicCast<WifiNetDevice>(nodeDevices.Get(i));

      Ptr<WifiMac> mac = wifi_dev->GetMac();
      //mac->SetAckTimeout(MilliSeconds(ackTimeOut));
      std::cout<<"Ack TimeOut Node["<<i<<"]: "<<mac->GetAckTimeout()<<std::endl<<std::endl;
      ...
      dca->SetMaxCw(y);
      std::cout<<i<<" dca maxcw: "<<dca->GetMaxCw()<<"mincw: "<<dca->GetMinCw()<<std::endl;
      //dca->SetAifsn(z);
	  ...
	  edca->SetMinCw(x);
	  edca->SetMaxCw(y);
      std::cout<<i<<" edca maxcw: "<<edca->GetMaxCw()<<"mincw: "<<edca->GetMinCw()<<std::endl;
	  //edca->SetAifsn(z);
 ...
 */
  ...
  ipAddrs.SetBase ("192.168.0.0", "255.255.192.0"); //.240 pour avoir plus de hosts dans le LAN que 256 (4094) .192 = 16000 hosts
  Ipv4InterfaceContainer interfaces = ipAddrs.Assign (nodeDevices);

  //IP and MAC address and ARP Cache
  Ptr<Packet> dummy= Create<Packet>();
  Ipv4Header ipHeader;
  ns3::ArpCache::Ipv4PayloadHeaderPair ppair(dummy, ipHeader);
  for(int i=0; i<nbrNodes; i++) {
    Ptr<WifiNetDevice> node= DynamicCast<WifiNetDevice>(nodeDevices.Get(i));
    Mac48Address mac= Mac48Address::ConvertFrom(node->GetAddress());
    Ptr<Node> inode = NodeList::GetNode(i);
    Ptr<Ipv4> ipv= inode->GetObject<Ipv4>();
    Ipv4Address ipa= ipv->GetAddress(1,0).GetLocal();
    for(int j=0; j<nbrNodes; j++) {
      if (i != j) {
        Ptr<Node> inode2 = NodeList::GetNode(j);
        Ptr<Ipv4> ipv2= inode->GetObject<Ipv4>();
        Ptr<Ipv4Interface> ip_interf= ipv2->GetObject<Ipv4L3Protocol> ()->GetInterface(1);
        Ptr<ArpCache> arpc= ip_interf->GetArpCache();
        if (arpc == NULL) {
          arpc= CreateObject<ArpCache>();
        }
        arpc->SetAliveTimeout (Seconds(3600*24*365));
       // std::cout<<"Address IP: "<<ipa<<" | Address MAC: "<<mac<<std::endl<<std::endl;
        if (arpc->Lookup(ipa) == 0) {
          ArpCache::Entry* entry= arpc->Add(ipa);
          entry->MarkWaitReply(ppair);
          entry->MarkAlive(mac);
  ...
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");//ConstantPositionMobilityModel
  mobility.Install (nodes);

  Ptr<UniformRandomVariable> posGen = CreateObject<UniformRandomVariable>(); 
  ...
  for(int i=0;i<nbrNodes;i++)
  {
    ...
    {
      float r = 100 * sqrt(posGen->GetValue(0,1));
      float theta = 2 * M_PI * posGen->GetValue(0,1);
  ...
  NS_LOG_INFO ("Create traffic source & sink.");
                              ...
                              /*Ptr<Node> nodeSource = NodeList::GetNode (1);
                              Ptr<Sender> sender = CreateObject<Sender>();
                              nodeSource->AddApplication (sender);
                              sender->SetStartTime (Seconds (1));

                              //ab
                              Ptr<Node> nodeSource_two = NodeList::GetNode (2);
                              Ptr<Sender> sender_two = CreateObject<Sender>();
                              nodeSource_two->AddApplication (sender_two);
  ...
  for(int i=1;i<nbrNodes;i++)
  {
    ...
    Ptr<Node> nodeSource = NodeList::GetNode (i);

    Ptr<Sender> sender = CreateObject<Sender>();
    //Ptr<Receiver> receiver = CreateObject<Receiver>();

    nodeSource->AddApplication (sender);
    //nodeSource->AddApplication (receiver);

    sender->SetStartTime (Seconds (i-1));
    sender->SetStopTime (Seconds (i-1+0.5));
    
    
  }    

  //SeedManager::SetSeed (17);
  Ptr<UniformRandomVariable> gen = CreateObject<UniformRandomVariable>(); 
 ...
 /* for(int i=1;i<nbrNodes;i++)
  {
    ...
    Ptr<Node> nodeSource = NodeList::GetNode (i);
    //Ptr<Sender> sender = CreateObject<Sender>();
    Ptr<Receiver> receiverARP = CreateObject<Receiver>();

    nodeSource->AddApplication(receiverARP);
   // nodeSource->AddApplication (sender);

    double randStart = gen->GetValue(0,0.5);
    std::cout<<"Random("<<i<<")"<<randStart<<std::endl; 

    receiverARP->SetStartTime(Seconds(0));
    receiverARP->SetStopTime(Seconds(4));

   // nodeSource->GetApplication(1)->SetStartTime(Seconds(6));    //ajouter randStart+ avant 110

    std::cout<<"* Node["<<i<<"]"<<" Nombre d'applications: "<<nodeSource->GetNApplications()<<std::endl;
...
*/
  ...
  for(int i=1;i<nbrNodes;i++)
  {
    ...
    double randStart = gen->GetValue(0,0.5);//0.5
    std::cout<<"Random("<<i<<")"<<randStart<<std::endl; 

    //nodeSource->GetApplication(0)->SetStartTime(Seconds(0));
    //nodeSource->GetApplication(0)->SetStopTime(Seconds(4));

    sender->SetStartTime(Seconds(randStart+nodeStartTime));    //ajouter randStart+ avant nodeStartTime
 ...
 /* for(int i=1;i<nbrNodes;i++)
  {
    ...
    //double randStart = gen->GetValue(0,0.5);
    //std::cout<<"Random("<<i<<")"<<randStart<<std::endl; 
    sender->SetStartTime(Seconds(106));    //ajouter randStart+ avant 110
    nodeSource->AddApplication(sender);
    std::cout<<"** Node["<<i<<"]"<<" Nombre d'applications: "<<nodeSource->GetNApplications()<<std::endl;
  }    
  std::cout<<std::endl;  
  */
  ...
  Ptr<Node> nodeSink = NodeList::GetNode (0);
  Ptr<Receiver> receiver = CreateObject<Receiver>();
  //Ptr<Sender> senderARP = CreateObject<Sender>();

  //nodeSink->AddApplication(senderARP);
  nodeSink->AddApplication(receiver);

  //senderARP->SetStartTime(Seconds (0));
  //senderARP->SetStopTime(Seconds(4));


  //std::cout<<"** Node[0]"<<" Nombre d'applications: "<<nodeSink->GetNApplications()<<std::endl;

  
  for(int i=1;i<nbrNodes;i++)
  {
     Config::Set ("/NodeList/"+std::to_string(i)+"/ApplicationList/*/$Sender/Destination",
                     Ipv4AddressValue ("192.168.0.1"));
  }
  
  //for(int i=1;i<nbrNodes;i++)
  {
    // Config::Set ("/NodeList/"+std::to_string(i)+"/ApplicationList/*/$SenderARP/Destination",
                   //  Ipv4AddressValue ("192.168.0.1"));
  }
  ...
  //Config::Set ("/NodeList/"+std::to_string(0)+"/ApplicationList/"+std::to_string(0)+"/$SenderARP/Destination",
             //    Ipv4AddressValue ("192.168.0.2"));

  //ab
  /*Ptr<Node> nodeSink_two = NodeList::GetNode (2);
  Ptr<Receiver> receiver_two = CreateObject<Receiver>();
  nodeSink_two->AddApplication (receiver_two);
  ...
  for(int i=1;i<nbrNodes;i++)
  {
    Ptr<CounterCalculator<uint32_t> > totalTx =
      CreateObject<CounterCalculator<uint32_t> >();
    totalTx->SetKey ("wifi-tx-frames");
    totalTx->SetContext ("node["+std::to_string(i)+"] avec nodeID: "+std::to_string(NodeList::GetNode (i)->GetId()));
  ...
  **/
  ...
  //wifiPhy.EnablePcap ("csma-example-sim-abkDataLink", nodes);
  //k
  //------------------------------------------------------------
  //-- Run the simulation
  //--------------------------------------------
  ...
  if (format == "omnet") {
      NS_LOG_INFO (Simulator::Now ()<<" Creating omnet formatted data output.");
