...
//TxPower
  ...
  //CCA THRESHOLD
  for(int i=0;i<nbrNodes;i++)//à partir de 0 juste quand noeud zero est emetteur sinon de 1
  {
    Ptr<WifiNetDevice> ApDC= DynamicCast<WifiNetDevice>(nodeDevices.Get(i));
    Ptr<WifiPhy> wfManager=ApDC->GetPhy();
    wfManager->SetCcaMode1Threshold(5000);               //ligne à mettre en commentaire pour activer le CCA (aloha -> csma) + la boucle for suivante
    std::cout<<"ccaMode1Threshold Node["<<i<<"]: "<<wfManager->GetCcaMode1Threshold()<<std::endl<<std::endl;
  }
  ...
  for(int i=0;i<nbrNodes;i++)
  {
    ...
    else //if(i<(nbrNodes/2)) 
    { 
      //methode 1 
      /*float r1=posGen->GetValue(0,1);
      float r2=posGen->GetValue(0,1);
      float x, y;
